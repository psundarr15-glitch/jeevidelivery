...
class DeliveryChatScreen extends StatefulWidget {
  ...
  const DeliveryChatScreen({super.key, required this.orderId, this.customerName});
  @override State<DeliveryChatScreen> createState() => _DeliveryChatScreenState();
}

class _DeliveryChatScreenState extends State<DeliveryChatScreen> {
  final s = DeliveryChatService();
  ...
  Future<void> _connect() async {
    try {
      await s.connect(widget.orderId);
      await s.markRead();
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Chat • ${widget.customerName ?? 'Customer'}')),
      body: loading
              ...
              : Column(
                  children: [
                        ...
                        builder: (context, snap) {
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, i) {
                              ...
                              final mine = m['sender']?.toString() == 'delivery';
                              return Align(
                    ...
                    SafeArea(
                      child: Row(
                        children: [
                          Expanded(child: TextField(controller: c, minLines: 1, maxLines: 4, decoration: const InputDecoration(hintText: 'Message customer...', contentPadding: EdgeInsets.symmetric(horizontal: 16)))),
